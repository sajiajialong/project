...
In this blog, we will focus on the true distribution of the usage of e-cigarettes among youth. And pay greate attention to the reason for choosing e-cigarettes. By doing this, we found that prohibiting e-cigarettes by telling them e-cigarettes are harmful is not effective!
...
ggplot(data = regular[regular$key=="e"&regular$days>0,])+
  ...
  theme_bw()
```
The figure above shows that in 2014, the average days people spend in smoking was 3 days. Afterwards it gradually increases to 5 days in 2018. The increase of number of smokers with the middle levelof addiction or even higher is more obvious, though it is masked by the majority with low-level addiction.
